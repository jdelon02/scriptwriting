...
- `scripts/push_to_rag.py` ingests OKF concept bodies through LiteLLM `embedding-model` into the local `.env`'s `primary_vector_store_id`. `.githooks/pre-commit` refreshes OKF, stages changed navigation indexes, and uploads when configured; an empty ID skips it. Markdown must be fully staged first. This is an append-only prototype with local `.rag/` deduplication, not replacement/deletion sync. See `docs/rag-ingestion.md` for credentials, configuration, and recovery.
